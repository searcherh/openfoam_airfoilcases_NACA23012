...
ChordLength=1.0
airfoilfile='naca23012.dat' # the airfoil datas form website in .dat format
DomainHeight=20.0  # Multiples Chord Length
WakeLength=20.0 	 # Multiples Chord Length
...
LeadingEdgeGrading=2.0
TrailingEdgeGrading=0.8
inletGradingFactor=0.5
TrailingBlockAngle=5.0    	# Degrees



NBL=int(np.rint(math.log(1.0-(BLHeight/firstLayerHeight*(1.0-growthRate)))/math.log(growthRate)))
MaxLayerThickness=firstLayerHeight*growthRate**NBL
BLGrading=MaxLayerThickness/firstLayerHeight
LFF=(DomainHeight/2.0-BLHeight)
NFF=int(np.rint(math.log(MaxCellSize/MaxLayerThickness)/math.log(1.0-MaxLayerThickness/LFF+MaxCellSize/LFF)))
farfieldGrowthRate=(MaxCellSize/MaxLayerThickness)**(1.0/NFF)
FFGrading=MaxCellSize/MaxLayerThickness
NFFA=int(np.rint(3.14159/2.0*DomainHeight/2.0/MaxCellSize)) 
...
rSW=LeadingEdgeGrading**(1.0/NFFA)
...
rNW=LeadingEdgeGrading**(1.0/NFFA)
if LeadingEdgeGrading!=1:
...
rInletGrading=InletGrading**(1.0/NFFA)
...
rNWBL=LeadingEdgeGrading**(1.0/NFFA)
if LeadingEdgeGrading!=1:
...
rSWBL=LeadingEdgeGrading**(1.0/NFFA)
if LeadingEdgeGrading!=1:
